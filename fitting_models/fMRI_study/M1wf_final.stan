...
model {
  ...
  // the conflict only model 
    ...
    if ((t % TBlock)==1) {   //operator % in stan is modulus, and 1 mod 15 = 1 , 15 mod 15 = 0, 16 mod 15 = 1 etc It serves to detect the beginning of each block
      ...
      choice[i,t] ~ categorical_logit( tau[i] * (ev) ); // this establishes that the actual choice (as provided in data) should follow the categorical logit regression with temperature * expected values. 
      // prediction error 
      PE  =(outcomeM[i,t]*(wf[i]) + outcomeS[i,t] * (1-wf[i])) - ev[choice[i,t]]; //i.e. actual reward or loss - expected value of the choice. Here we combine the value of Money and Shock in one entity, and one of them (both is unnecessary) is weighted for a subject variable wf[i]
