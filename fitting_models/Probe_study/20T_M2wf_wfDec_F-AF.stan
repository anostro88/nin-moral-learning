...
model {
    ...
    evS = initV; // setting both to zero before going through the trials
      ...
      for (t in 1:(Tsubj[i])) { //sets a loop going through Tsubj=number of trials of that participant 
       ...
       if (filter_money[i,t] == 0) {   //set the EVM to 0 then the outM is missing
        evM[1] = 0;  // avEVM;
        evM[2] = 0;  // avEVM;
      }
       if (filter_shock[i,t] == 0) {   //set the EVM to 0 then the outM is missing
        evS[1] = 0;  // avEVS;
        evS[2] = 0;  // avEVS;
      }
      // choice
        choice[i,t] ~ categorical_logit(tau[i] * (filter_money[i,t]*evM*(wf[i]) + filter_shock[i,t]*evS*(1 - wf[i]) + (1-filter_money[i,t])*evS*wf[i] + (1 - filter_shock[i,t])*evM*(1-wf[i])));
      // // choice
      // if (filter_money[i,t] == 0) {
      //    choice[i,t] ~ categorical_logit(tau[i] * evS);
      // } else if (filter_shock[i,t] == 0){
      //   choice[i,t] ~ categorical_logit(tau[i] * evM);
      // } else {
      //   choice[i,t] ~ categorical_logit(tau[i] * (filter_money[i,t]*evM*(wf[i]) + filter_shock[i,t]*evS*(1 - wf[i]) + (1-filter_money[i,t])*evS*wf[i] + (1 - filter_shock[i,t])*evM*(1-wf[i])));
      // }
      
       // prediction error 
       PEM = filter_money[i,t] * (outcomeM[i,t] - evM[choice[i,t]]); 
       PES = filter_shock[i,t] * (outcomeS[i,t] - evS[choice[i,t]]); 
...
generated quantities {
  ...
  { for (i in 1:N) {
   ...
   //  if (filter_money[i,t] == 0) {
   //    log_lik[i,t] = log_lik[i,t] + categorical_logit_lpmf(choice[i,t] | tau[i] * evS);
   //    log_lik_sub[i] = log_lik_sub[i] + categorical_logit_lpmf(choice[i,t] | tau[i] * evS);
   //  } else if (filter_shock[i, t] == 0){
   //    log_lik[i,t] = log_lik[i, t] + categorical_logit_lpmf(choice[i,t] | tau[i] * evM);
   //    log_lik_sub[i] = log_lik_sub[i] + categorical_logit_lpmf(choice[i,t] | tau[i] * evM);
   //  } else {
   //    log_lik[i,t] = log_lik[i,t] + categorical_logit_lpmf(choice[i,t] | tau[i] * (filter_money[i,t]*evM*(wf[i]) + filter_shock[i,t]*evS*(1 - wf[i]) + (1-filter_money[i,t])*evS*wf[i] + (1 - filter_shock[i,t])*evM*(1-wf[i])));
   //      log_lik_sub[i] = log_lik_sub[i] + categorical_logit_lpmf(choice[i,t] | tau[i] * (filter_money[i,t]*evM*(wf[i]) + filter_shock[i,t]*evS*(1 - wf[i]) + (1-filter_money[i,t])*evS*wf[i] + (1 - filter_shock[i,t])*evM*(1-wf[i])));
   //  }
   ...
   y_pred[i,t] = categorical_rng(softmax(tau[i] * (filter_money[i,t]*evM*(wf[i]) + filter_shock[i,t]*evS*(1 - wf[i]) + (1-filter_money[i,t])*evS*wf[i] + (1 - filter_shock[i,t])*evM*(1-wf[i]))));
   //  if (filter_money[i,t] == 0) {
   //    y_pred[i,t] = categorical_rng(softmax(tau[i] * evS));
   //    } else if (filter_shock[i,t] == 0){
   //    y_pred[i,t] = categorical_rng(softmax(tau[i] * evM));
   //    } else {
   //    y_pred[i,t] = categorical_rng(softmax(tau[i] * (filter_money[i,t]*evM*(wf[i]) + filter_shock[i,t]*evS*(1 - wf[i]) + (1-filter_money[i,t])*evS*wf[i] + (1 - filter_shock[i,t])*evM*(1-wf[i]))));
   //    }
