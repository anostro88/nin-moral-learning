...
model {
    ...
    evS = initV; // setting both to zero before going through the trials
     ...
     for (t in 1:(Tsubj[i])) { //sets a loop going through Tsubj=number of trials of that participant 
      ...
      // choice
      choice[i,t] ~ categorical_logit(tau[i] * (filter_money[i,t]*evM + filter_shock[i,t]*evS));
      // prediction error 
      PEM = (filter_money[i,t] * outcomeM[i,t] * wf[i] + (1 - filter_shock[i,t]) * outcomeM[i,t] * (1-wf[i])) - evM[choice[i,t]]; 
      PES = (filter_shock[i,t] * outcomeS[i,t] * (1- wf[i]) + (1 - filter_money[i,t]) * outcomeS[i,t] * (wf[i])) - evS[choice[i,t]]; 
...
generated quantities {
  ...
  { for (i in 1:N) {
   ...
   log_lik[i,t] = log_lik[i,t] + categorical_logit_lpmf(choice[i,t] | tau[i] * (filter_money[i,t]*evM + filter_shock[i,t]*evS));
   log_lik_sub[i] = log_lik_sub[i] + categorical_logit_lpmf(choice[i,t] | tau[i] * (filter_money[i,t]*evM + filter_shock[i,t]*evS));
   ...
   y_pred[i,t] = categorical_rng(softmax(tau[i] * (filter_money[i,t]*evM + filter_shock[i,t]*evS)));
