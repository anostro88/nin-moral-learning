data {
  ...
  int<lower=1> trial; //max number of regular trials per subject 
  int<lower=1> triald; // max number of trials with dropout per subject 
  int<lower=1> TBlock; //number of trials before new symbols are used
  int<lower=1> TDBlock;
  int<lower=1,upper=trial> Tsubj[N]; //vector containing the number of trials for each subject
  int<lower=1,upper=triald> TDsubj[N];
  int<lower=1,upper=2> choice[N,trial]; //Matrix of N subjects x T trials of 1 and 2 
  int<lower=1,upper=2> choice_drop[N,triald];
  real outcomeM[N,trial];  // Matrix of N subjects x T trials with no lower and upper bounds, indicating the Money incurred on ever trial.   
  real outcomeM_drop[N,triald]; 
  real outcomeS[N,trial];  // Matrix of N subjects x T trials with no lower and upper bounds, indicating the Shock incurred on ever trial.   
  real outcomeS_drop[N,triald]; 
  real filter_money[N,triald]; // Matrix of Nsub x T trials with 1 = reward outcome and 0 = no reward outcome
  real filter_shock[N,triald]; //// Matrix of Nsub x T trials with 1 = shock outcome and 0 = no shock outcome
...
model {
  ...
  // the conflict only model 
    ...
    if ((t % TBlock)==1) {   //operator % in stan is modulus, and 1 mod 15 = 1 , 15 mod 15 = 0, 16 mod 15 = 1 etc It serves to detect the beginning of each block
        ...
        // prediction error 
        PE = outcomeM[i,t]*(wf[i]) + outcomeS[i,t]*(1-wf[i]) - ev[choice[i,t]];
...
// this includes the population A and tau, that are properly scaled, as well as a cumulative log_lik over all trails for each participant. 
  ...
  real tbtPE[N,triald];
  real tbtevc[N,triald];
  real tbtev1[N, triald];
  real tbtev2[N,triald];
  
  // For log likelihood calculation
  real log_lik[N,triald]; // for the conflict choices
  real log_lik_sub[N]; // for the conflict choices

// For posterior predictive check
  real y_pred[N,triald]; 
  ...
  // Define the conflict model 
      ...
      for (t in 1:(TDsubj[i])) {   // loop for every trial of each participant
      
         log_lik[i,t] = 0;
      
      if ((t % TDBlock)==1) {   //operator% in stan is modulus, and 1 mod 15 = 1 , 15 mod 15 = 0, 16 mod 15 = 1 etc 
        avEV=(ev[1]+ev[2])/2;
        ev[1]=avEV;ev[2]=avEV;
      }
        ...
        log_lik[i,t] = log_lik[i,t] + categorical_logit_lpmf(choice_drop[i,t] | tau[i] * (ev));       
        log_lik_sub[i] = log_lik_sub[i] + categorical_logit_lpmf(choice_drop[i,t] | tau[i] * (ev));       

        // prediction error 
        PE = (filter_money[i,t]*outcomeM_drop[i,t]*(wf[i]) + filter_shock[i,t]*outcomeS_drop[i,t]*(1-wf[i]) + (1-filter_money[i,t])*outcomeS_drop[i,t]*wf[i] + (1-filter_shock[i,t])*outcomeM_drop[i,t]*(1-wf[i])) - ev[choice_drop[i,t]];
        ...
        // value updating (learning) 
        ev[choice_drop[i,t]] = ev[choice_drop[i,t]] + A[i] * PE;
        tbtev1[i,t]=ev[1];
        tbtev2[i,t]=ev[2];
        tbtevc[i,t]= ev[choice_drop[i,t]];
